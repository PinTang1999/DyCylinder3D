...
class Asymm_3d_spconv(nn.Module):
    def __init__(self,
        ...
        # self.fuse = nn.Linear(4 * init_size, 4 * init_size)
        # self.bn1 = nn.BatchNorm1d(4 * init_size)
        # self.act1 = nn.ReLU()

        self.conv_before_seg = spconv.SubMConv3d(4 * init_size, 2 * init_size, indice_key="conv_before_seg",
                                                 kernel_size=3, stride=1, padding=1, bias=True)

        self.seg = spconv.SubMConv3d(2 * init_size, nclasses, indice_key="seg", kernel_size=1, bias=False)
    ...
    def forward(self, voxel_features, coors, batch_size):
        ...
        # up0e.features = self.act1(self.bn1(self.fuse(up0e.features)))

        upend = self.conv_before_seg(up0e)
        logits = self.seg(upend)
        # logits.dense(): a tensor with size of sparse_shape, i.e., the segmentation results on the voxel level
        y = up0e.features, logits.features, logits.dense()
        return y


class DynamicSparse3D(nn.Module):
    def __init__(self,
                 output_shape,
                 use_norm=True,
                 num_input_features=128,
                 nclasses=20, n_height=32, strict=False, init_size=16):
        super(DynamicSparse3D, self).__init__()
        self.nclasses = nclasses
        self.nheight = n_height
        self.strict = False
        ...
        self.conv_before_seg = spconv.SubMConv3d(4 * init_size, 4 * init_size, indice_key="conv_before_seg",
                                                 kernel_size=3, stride=1, padding=1, bias=True)

        # self.seg = spconv.SubMConv3d(2 * init_size, nclasses, indice_key="seg", kernel_size=1)
        self.seg = nn.Linear(4 * init_size, nclasses, bias=False)
    ...
    def forward(self, voxel_features, coors, batch_size):
        ...
        upend = self.conv_before_seg(up0e)
        voxel_features = upend.features  # [m, 4*init_size]
        upend.features = self.seg(upend.features)  # [m, cls]
        seg_weights = self.seg.weight.clone()  # [cls, 4*init_size]

        # voxel_features [M, C],
        # upend.features [M, cls]
        # upend.dense() [BS, W, H, Z]
        # seg_weights [cls, C]
        y = voxel_features, upend.features, upend.dense(), seg_weights
        return y
